...
# MAGIC %md
# MAGIC # Weather News -> Vector Embeddings (Lakebase)
# MAGIC
# MAGIC This notebook is part of the **Context Engineering on Databricks** course.
# MAGIC
# MAGIC It:
# MAGIC 1. Reads the `weather_documents` table in Lakebase to find out which ticker
# MAGIC    symbols are currently being tracked.
# MAGIC 2. Fetches recent news for those tickers directly from the Weather
# MAGIC    `/v2/reference/news` endpoint (see `weather_client.py` for the same
...
dbutils.widgets.text("weather_table_name", "weather_documents", "Source table (weather_documents)")
...
# MAGIC %md
# MAGIC ## Fetch weather news from NWS
# MAGIC
# MAGIC This ETL is now self-contained: instead of relying on the Flask app's
# MAGIC `POST /news/sync` route to have populated `weather_documents`.
# MAGIC
# MAGIC The free Weather API tier is rate-limited very aggressively, so requests
...
def get_watchlist_tickers() -> list[str]:
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(f"SELECT DISTINCT symbol FROM {WEATHER_TABLE_NAME} WHERE symbol IS NOT NULL")
...
def fetch_news_for_ticker(session: requests.Session, ticker: str, limit: int) -> list[dict]:
    ...
    WeatherClient.get_news in massive_client.py)."""
    resp = session.get(
